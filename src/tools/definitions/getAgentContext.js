...
export const name = 'get_agent_context';
export const description = 'Prepare agent context for parallel section implementation. Call after analyze_page_structure. Returns section details, responsibilities, assets, styles, and agent-specific instructions.';
...
export const inputSchema = {
  ...
  agent_index: z.number().optional().default(0).describe('Zero-based agent index (default: 0)'),
  total_agents: z.number().optional().default(1).describe('Total number of agents working in parallel (default: 1)'),
};
...
export async function handler(args, ctx) {
  const { file_key, page_name, frame_name, section_id, agent_index, total_agents } = args;
  ...
  const sectionIndex = parseInt(section_id.split('-')[1], 10);
  if (sectionIndex < 0 || sectionIndex >= sectionGroups.length) {
    throw new Error(`Invalid section ID: ${section_id}. Available: section-0 to section-${sectionGroups.length - 1}`);
  }
  ...
  const enrichedAssets = {
    icons: sectionAssets.icons.map((a) => buildAssetDetails(a, file_key, sectionName)),
    images: sectionAssets.images.map((a) => buildAssetDetails(a, file_key, sectionName)),
  };
  ...
  for (const element of transitionElements) {
    if (element.spansSections.includes(section_id)) {
  ...
  const agentInfo = {
    index: agent_index,
    total: total_agents,
    isFirst: agent_index === 0,
    isLast: agent_index === total_agents - 1,
  };
  ...
  const response = chunker.wrapResponse(result, {
    step: `Prepared context for Agent ${agent_index}`,
    progress: `${section.name} section (${agent_index + 1}/${total_agents})`,
